...
function setupButtonListeners() {
  ...
  UI.startBtn.addEventListener('click', async () => {
    ...
    if (success) {
      ...
      window.chatManager.addSystemMessage('Screen sharing started. AI assistant is now watching.');
    }
  });
  ...
  UI.saveSessionBtn.addEventListener('click', async () => {
    ...
    try {
      ...
      window.chatManager.addSystemMessage('❌ Failed to save session. Please try again.');
      alert('Failed to save session. Please check your connection to the backend.');
    }
  });
  ...
  UI.clearChatBtn.addEventListener('click', () => {
    ...
    if (confirm('Are you sure you want to clear all chat messages?')) {
      ...
      window.chatManager.addSystemMessage('Chat cleared. Start a new conversation!');
    }
  });
}
...
function handleConnectionStatus(status) {
  ...
  switch (status) {
    ...
    case 'failed':
      ...
      UI.connectionStatus.style.color = '#dc3545';
      window.chatManager.addSystemMessage('⚠️ Connection failed. Please refresh the page.');
      break;
  }
}
